...
export default function PasswordInput({ id, label, value, onChange, autoComplete = "new-password" }: {
  ...
  return (
    <div>
      ...
      <div className="relative">
        ...
        <input id={id} type={visible ? "text" : "password"} value={value} onChange={(event) => onChange(event.target.value)} autoComplete={autoComplete} className="input pl-10 pr-11" placeholder={isEnglish ? "Enter password" : "Masukkan kata sandi"} />
        <button type="button" onClick={() => setVisible((current) => !current)} className="absolute right-2 top-1/2 grid size-8 -translate-y-1/2 place-items-center rounded-lg text-slate-500 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 dark:hover:bg-slate-800" aria-label={visible ? (isEnglish ? "Hide password" : "Sembunyikan kata sandi") : (isEnglish ? "Show password" : "Tampilkan kata sandi")}>
