...
type Props = { id: string; disabled?: boolean; onFile: (file: File) => void };
...
export default function ImageDropzone({ id, disabled, onFile }: Props) {
  ...
  return (
    <div role="button" tabIndex={disabled ? -1 : 0} aria-disabled={disabled}
      onClick={() => !disabled && inputRef.current?.click()} onKeyDown={onKeyDown}
      ...
      <span className="mx-auto grid size-12 place-items-center rounded-2xl bg-white text-indigo-600 shadow-sm ring-1 ring-slate-200 transition group-hover:scale-105 dark:bg-slate-900 dark:ring-slate-700">{dragging ? <ImagePlus className="size-6" /> : <UploadCloud className="size-6" />}</span>
      <p className="mt-4 font-semibold text-slate-800 dark:text-slate-100">{isEnglish ? "Drop a PNG image here" : "Tarik gambar PNG ke sini"}</p>
      <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{isEnglish ? "or click to browse • maximum 15 MB" : "atau klik untuk memilih • maksimum 15 MB"}</p>
    </div>
  );
}
